...
# เพิ่มตัวแปร CHANGELOG
CHANGELOG = {
    "0.3": [
        "เพิ่มฟังก์ชันการอัพเดตสคริปต์จาก GitHub",
        "ปรับปรุง UI ให้ดูสวยงามและไฮเทคขึ้น",
        "เพิ่มแอนิเมชันกำลังอัพเดตที่ใช้เวลา 23 วินาที",
        "ปรับปรุงการจัดเรียงเมนูให้คำสั่งออกจากโปรแกรมอยู่ท้ายสุด",
        "ปรับปรุงการจัดการความผิดพลาดให้ดีขึ้น"
    ],
    "0.2": [
        "แก้ไขข้อผิดพลาดในการอัพเดตเวอร์ชัน",
        "ปรับปรุงการแสดงผลข้อความในเมนูหลัก"
    ],
    "0.1": [
        "เริ่มต้นโปรเจกต์",
        "เพิ่มฟังก์ชันสร้างโค้ด VLESS",
        "เพิ่มฟังก์ชันดูจำนวนผู้ใช้งานออนไลน์"
    ]
}
...
def update_script():
    ...
    try:
        ...
        if latest_version > VERSION:
            # แสดงรายละเอียดการเปลี่ยนแปลงก่อนการยืนยันอัพเดต
            console.print(f"[bold yellow]🚀 เวอร์ชั่นใหม่ {latest_version} มีการเปลี่ยนแปลงดังนี้:[/bold yellow]")
            for change in changelog.get(latest_version, []):
                console.print(f"• {change}")
            console.print("\n[bold yellow]คุณต้องการอัพเดตโปรแกรมใช่หรือไม่?[/bold yellow]")
    ...
    if 'latest_version' in locals() and latest_version > VERSION:
