...
GITHUB_REPO_API = "https://api.github.com/repos/Eyegff/EPVFFOTR.0.1"
GITHUB_RAW_SCRIPT_URL = "https://raw.githubusercontent.com/Eyegff/EPVFFOTR.0.1/main/EXCELLENT.py"  # เปลี่ยนเป็น path ของสคริปต์ใน repo
GITHUB_VERSION_URL = "https://raw.githubusercontent.com/Eyegff/EPVFFOTR.0.1/main/version.json"  # ไฟล์เวอร์ชันใน repo
...
def get_login_details():
    ...
    password = Prompt.ask("[bold cyan]🔒 กรุณาใส่รหัสผ่าน (password)")  # แสดงรหัสผ่านขณะพิมพ์
...
def update_script():
    ...
    try:
        # ดึงข้อมูลเวอร์ชันล่าสุดจาก GitHub
        version_response = requests.get(GITHUB_VERSION_URL)
        version_response.raise_for_status()
        latest_version_info = version_response.json()
        latest_version = latest_version_info.get("version")

        if not latest_version:
            console.print("[bold red]❌ ไม่พบข้อมูลเวอร์ชันล่าสุดจาก GitHub[/bold red]")
            console.input("[bold green]กด Enter เพื่อกลับสู่เมนูหลัก...[/bold green]")
            return

        # เปรียบเทียบเวอร์ชันปัจจุบันกับเวอร์ชันล่าสุด
        current_version = VERSION
        if latest_version > current_version:
            console.print(f"[bold yellow]🚀 มีเวอร์ชันใหม่: {latest_version}[/bold yellow]")
            update_choice = Prompt.ask("[bold cyan]ต้องการอัพเดตโปรแกรมใช่หรือไม่? (y/n)", choices=["y", "n"], default="n")
            if update_choice.lower() == 'y':
                console.print("[bold green]🔄 กำลังดาวน์โหลดเวอร์ชันใหม่...[/bold green]")
                # ดาวน์โหลดสคริปต์ล่าสุด
            ...
            console.print("[bold green]✅ โปรแกรมของคุณเป็นเวอร์ชันล่าสุด[/bold green]")
    except requests.exceptions.RequestException as e:
...
[bold magenta]4️⃣  เปลี่ยนแปลงข้อมูลการล็อกอิน[/bold magenta]
[bold yellow]5️⃣  เพิ่มจำนวนเครือข่าย[/bold yellow]
[bold cyan]6️⃣  อัพเดตสคริปต์[/bold cyan]
    """, style="bold blue", padding=(2,4)))

def create_update_option(config):
    """ฟังก์ชันสำหรับเพิ่มอัพเดตสคริปต์เข้ามาในเมนู"""
    pass  # Placeholder ในกรณีต้องการเพิ่มฟังก์ชันเพิ่มเติม
