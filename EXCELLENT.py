...
def get_login_details():
    ...
    base_url = Prompt.ask("[bold cyan]🔗 กรุณาใส่ลิ้งล็อกอิน (base_url)", default="https://example.com")
    username = Prompt.ask("[bold cyan]📝 กรุณาใส่ชื่อผู้ใช้ (username)", default="admin")
    ...
    for i in range(1, num_carriers + 1):
        ...
        name = Prompt.ask(f"[bold cyan]🔖 กรุณาตั้งชื่อเครือข่ายที่ต้องการ (ตัวอย่าง: ทรูโปรเฟส)", default=f"เครือข่าย {i}")
        ...
        example_code = Prompt.ask(f"[bold cyan]🔍 กรุณาใส่โค้ดตัวอย่างของ {name} (ตัวอย่าง: vless://...)", default="vless://example_code")
        carriers.append({
            "name": name,
            "id": carrier_id,
...
def add_carriers():
    ...
    for i in range(1, num_carriers + 1):
        console.print(f"\n[bold yellow]เครือข่ายที่ {i}[/bold yellow]")
        name = Prompt.ask(f"[bold cyan]🔖 กรุณาตั้งชื่อเครือข่ายที่ต้องการ (ตัวอย่าง: ทรูโปรเฟส)", default=f"เครือข่ายใหม่ {i}")
        id_input = get_validated_input(
            f"[bold cyan]🔢 กรุณาใส่ ID ของ {name} (ตัวอย่าง:5)",
            default="5",
            validation_func=lambda x: x.isdigit(),
...
[/bold cyan]
    ...
    # รับชื่อโค้ด
    code_name = Prompt.ask("[bold cyan]📝 ตั้งชื่อโค้ดของคุณ[/bold cyan]", default="โค้ดใหม่")
        ...
        with console.status("[bold green]⏳ กำลังประมวลผล..."):
            ...
            if result.get("success"):
                ...
                if total_online > 0:
                    ...
                    table = Table(title="รายชื่อผู้ใช้งานออนไลน์", box=box.ROUNDED, style="bold cyan")
                    table.add_column("ลำดับ", justify="right", style="cyan", no_wrap=True)
                    ...
                    console.print("[bold yellow]⚠️ ไม่มีผู้ใช้งานออนไลน์[/bold yellow]")
            else:
...
def update_script():
    ...
    try:
        ...
        console.print(f"[bold yellow]{PROGRAM_NAME}[/bold yellow] - เวอร์ชั่น [bold green]{VERSION}[/bold green]")

        if latest_version > VERSION:
            console.print(f"[bold yellow]{PROGRAM_NAME}[/bold yellow] - เวอร์ชั่น [bold green]{latest_version}[/bold green] มีเวอร์ชั่นใหม่ กรุณายืนยันที่จะอัพเดท กด [bold cyan]y[/bold cyan] เพื่อยืนยันหรือ [bold cyan]n[/bold cyan] เพื่อยกเลิก")
            ...
            if update_choice.lower() == 'y':
                ...
                # แสดงแอนิเมชันกำลังอัพเดทเป็นเวลา 5 วินาที
                with Progress(
                    SpinnerColumn(style="bold green"),
                    TextColumn("[progress.description]{task.description}"),
                    BarColumn(),
                    ...
                    task = progress.add_task("[bold blue]กำลังอัพเดทโปรแกรม...[/bold blue]", total=5)
                    for _ in range(5):
                        time.sleep(1)
                        progress.update(task, advance=1)
...
[bold red]6️⃣  ออกจากโปรแกรม[/bold red]
    """, vertical="middle"), style="bold blue", padding=(2,4), box=box.ROUNDED)
